...
| `DRY_RUN` | No | `false` | Log CLI commands without executing |
...
Set `DRY_RUN=true` in `.env.local` to test the agent without executing real CLI commands. GPT will still reason about tools, but the executor will log commands instead of running them.
